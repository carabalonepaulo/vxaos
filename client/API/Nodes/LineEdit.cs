...
namespace API.Nodes
{
    public class LineEdit : Godot.LineEdit, IRubyControl
    {
        ...
        public LineEdit(object rubyOwner)
        {
        ...
        #region LineEdit
        void OnTextChangeRejected(string rejectedSubstring) => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "text_change_rejected", rejectedSubstring);
        void OnTextChanged(string newText) => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "text_changed", newText);
        void OnTextEntered(string newText) => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "text_entered", newText);
        #endregion
    }
}