...
namespace API.Nodes
{
    public class ItemList : Godot.ItemList, IRubyControl
    {
        ...
        public ItemList(object rubyOwner)
        {
        ...
        void OnItemRMBSelected(int index, Vector2 atPosition) => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "item_rmb_selected", index, atPosition);
        void OnItemSelected(int index) => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "item_selected", index);
        void OnMultiSelected(int index, bool selected) => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "multi_selected", index, selected);
        void OnNothingSelected() => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "nothing_selected");
        void OnRMBClicked(Vector2 atPosition) => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "rmb_clicked");
        #endregion
    }
}