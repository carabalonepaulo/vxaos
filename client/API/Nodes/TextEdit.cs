...
namespace API.Nodes
{
    public class TextEdit : Godot.TextEdit, IRubyControl
    {
        ...
        public TextEdit(object rubyOwner)
        {
        ...
        void OnInfoClicked(int row, string info) => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "info_clicked", row, info);
        void OnRequestCompletion() => RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "request_completion");
        void OnSymbolLookup(string symbol, int row, int column) =>
            RubyEnvironment.Engine.Operations.InvokeMember(Emitter, "emit", "symbol_lookup", symbol, row, column);
